...
// TODO: implement
import { Database } from "obsidian-database-library-main";
  ...
  async onload() {
    ...
    this.registerEvent(
      this.app.workspace.on("file-open", (file) =>
        this.restoreEphemeralState(file),
      ),
    );
  ...
  async restoreEphemeralState(file?: TFile | null) {
    ...
    if (
      ...
      this.loadedLeafIdList.includes(
        ...
        (activeLeaf as any).id + ":" + activeLeaf.getViewState().state?.file,
      )
    )
      return;
    ...
    this.app.workspace.iterateAllLeaves((leaf) => {
      if (leaf.getViewState().type === "markdown") {
        this.loadedLeafIdList.push(
          // @ts-ignore unofficial API
          (leaf as any).id + ":" + leaf.getViewState().state?.file,
        );
      }
    });
      ...
      if (fileName) {
        ...
        if (saved) {
          ...
          let containsFlashingSpan =
            this.app.workspace.containerEl.querySelector(".is-flashing");
