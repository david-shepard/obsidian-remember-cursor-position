...
export class Database<T> extends EventComponent {
	...
	constructor(
		...
		this.persist = localforage.createInstance({
			// @ts-ignore unofficial API
			name: this.name + `/${(this.plugin.app as any).appId}`,
					...
					if (file instanceof TFile && file.extension === "md") {
						...
						const state =
							(current_editor && current_editor.file?.path === file.path && current_editor.editor) ?
								// @ts-ignore unofficial API
						(current_editor.editor as any).cm.state :
								undefined;
	...
	async loadDatabase() {
		this.memory = new Map(
			Object.entries(await this.persist.getItems() as Record<string, DatabaseItem<T>>)
				.map(([key, value]: [string, DatabaseItem<T>]) => {
	 ...
	 */
	async dropDatabase() {
		const appId = (this.plugin.app as any).appId;
		this.memory.clear();
		await localforage.dropInstance({
			name: this.name + `/${appId}`,
		});
		localStorage.removeItem(appId + "-" + this.name + "-version");
	}
